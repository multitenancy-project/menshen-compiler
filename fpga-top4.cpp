...
Visitor::profile_t FPGAToP4::init_apply(const IR::Node* node) {
...
void FPGAToP4::end_apply(const IR::Node*) {
...
bool FPGAToP4::isSystemFile(cstring file) {
    if (noIncludes) return false;
    if (file.startsWith(p4includePath)) return true;
    return false;
}

cstring FPGAToP4::ifSystemFile(const IR::Node* node) {
...
unsigned FPGAToP4::curDepth() const {
    unsigned result = 0;
    auto ctx = getContext();
    while (ctx != nullptr) {
        ctx = ctx->parent;
...
void FPGAToP4::dump(unsigned depth, const IR::Node* node, unsigned adjDepth) {
    if (!showIR)
        return;
    if (node == nullptr)
        node = getOriginal();
...
bool FPGAToP4::preorder(const IR::P4Program* program) {
...
bool FPGAToP4::preorder(const IR::Type_Bits* t) {
    if (t->expression) {
        builder.append("bit<(");
        visit(t->expression);
        builder.append(")>");
...
bool FPGAToP4::preorder(const IR::Type_String* t) {
    builder.append(t->toString());
    return false;
}

bool FPGAToP4::preorder(const IR::Type_InfInt* t) {
    builder.append(t->toString());
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Var* t) {
    builder.append(t->name);
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Unknown*) {
    BUG("Cannot emit code for an unknown type");
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Dontcare*) {
    builder.append("_");
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Void*) {
    builder.append("void");
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Name* t) {
    visit(t->path);
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Stack* t) {
    dump(2);
    visit(t->elementType);
    builder.append("[");
    visit(t->size);
...
bool FPGAToP4::preorder(const IR::Type_Specialized* t) {
    dump(3);
    visit(t->baseType);
    builder.append("<");
    setVecSep(", ");
...
bool FPGAToP4::preorder(const IR::Argument* arg) {
...
bool FPGAToP4::preorder(const IR::Type_Typedef* t) {
    dump(2);
    visit(t->annotations);
...
bool FPGAToP4::preorder(const IR::Type_Newtype* t) {
    dump(2);
    visit(t->annotations);
    builder.append("type ");
    visit(t->type);
...
bool FPGAToP4::preorder(const IR::Type_BaseList* t) {
    dump(3);
...
bool FPGAToP4::preorder(const IR::P4ValueSet* t) {
    dump(1);
    visit(t->annotations);
...
bool FPGAToP4::preorder(const IR::Type_Enum* t) {
    dump(1);
    visit(t->annotations);
    builder.append("enum ");
    builder.append(t->name);
...
bool FPGAToP4::preorder(const IR::Type_SerEnum* t) {
    dump(1);
    visit(t->annotations);
    builder.append("enum ");
    visit(t->type);
...
bool FPGAToP4::preorder(const IR::TypeParameters* t) {
    if (!t->empty()) {
        builder.append("<");
        bool first = true;
...
bool FPGAToP4::preorder(const IR::Method* m) {
    dump(1);
    visit(m->annotations);
...
bool FPGAToP4::preorder(const IR::Function* function) {
    dump(1);
...
bool FPGAToP4::preorder(const IR::Type_Extern* t) {
    dump(2);
    if (isDeclaration) {
        visit(t->annotations);
        builder.append("extern "); }
...
bool FPGAToP4::preorder(const IR::Type_Boolean*) {
    builder.append("bool");
    return false;
}

bool FPGAToP4::preorder(const IR::Type_Varbits* t) {
    if (t->expression) {
...
bool FPGAToP4::preorder(const IR::Type_Package* package) {
    dump(2);
    builder.emitIndent();
...
bool FPGAToP4::process(const IR::Type_StructLike* t, const char* name) {
    dump(2);
    if (isDeclaration) {
        builder.emitIndent();
        visit(t->annotations);
...
bool FPGAToP4::preorder(const IR::Type_Parser* t) {
    dump(2);
    builder.emitIndent();
    visit(t->annotations);
...
bool FPGAToP4::preorder(const IR::Type_Control* t) {
    dump(2);
    builder.emitIndent();
    visit(t->annotations);
...
bool FPGAToP4::preorder(const IR::Constant* c) {
...
bool FPGAToP4::preorder(const IR::BoolLiteral* b) {
    builder.append(b->toString());
    return false;
}

bool FPGAToP4::preorder(const IR::StringLiteral* s) {
    builder.append(s->toString());
    return false;
}

bool FPGAToP4::preorder(const IR::Declaration_Constant* cst) {
    dump(2);
...
bool FPGAToP4::preorder(const IR::Declaration_Instance* i) {
    dump(3);
    visit(i->annotations);
...
bool FPGAToP4::preorder(const IR::Declaration_Variable* v) {
    dump(2);
    visit(v->annotations);
...
bool FPGAToP4::preorder(const IR::Type_Error* d) {
    dump(1);
    bool first = true;
...
bool FPGAToP4::preorder(const IR::Declaration_MatchKind* d) {
    dump(1);
...
#define VECTOR_VISIT(V, T)                                \
    bool FPGAToP4::preorder(const IR:: V <IR::T> *v) {        \
...
bool FPGAToP4::preorder(const IR::Slice* slice) {
...
bool FPGAToP4::preorder(const IR::DefaultExpression*) {
...
bool FPGAToP4::preorder(const IR::This*) {
    builder.append("this");
    return false;
}

bool FPGAToP4::preorder(const IR::PathExpression* p) {
    visit(p->path);
    return false;
}

bool FPGAToP4::preorder(const IR::TypeNameExpression* e) {
    visit(e->typeName);
    return false;
}

bool FPGAToP4::preorder(const IR::ConstructorCallExpression* e) {
...
bool FPGAToP4::preorder(const IR::Member* e) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::SelectCase* e) {
    dump(2);
    int prec = expressionPrecedence;
    expressionPrecedence = DBPrint::Prec_Low;
    setListTerm("(", ")");
...
bool FPGAToP4::preorder(const IR::SelectExpression* e) {
...
bool FPGAToP4::preorder(const IR::ListExpression* e) {
    cstring start, end;
...
bool FPGAToP4::preorder(const IR::NamedExpression* e) {
...
bool FPGAToP4::preorder(const IR::StructExpression* e) {
...
bool FPGAToP4::preorder(const IR::MethodCallExpression* e) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::Operation_Binary* b) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::Mux* b) {
    int prec = expressionPrecedence;
    bool useParens = prec >= b->getPrecedence();
    if (useParens)
        builder.append("(");
...
bool FPGAToP4::preorder(const IR::Operation_Unary* u) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::ArrayIndex* a) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::Cast* c) {
    int prec = expressionPrecedence;
...
bool FPGAToP4::preorder(const IR::AssignmentStatement* a) {
    dump(2);
    visit(a->left);
    builder.append(" = ");
    visit(a->right);
...
bool FPGAToP4::preorder(const IR::BlockStatement* s) {
    dump(1);
    visit(s->annotations);
    builder.blockStart();
    setVecSep("\n", "\n");
...
bool FPGAToP4::preorder(const IR::ExitStatement*) {
    dump(1);
    builder.append("exit");
    builder.endOfStatement();
    return false;
...
bool FPGAToP4::preorder(const IR::ReturnStatement* statement) {
    dump(2);
...
bool FPGAToP4::preorder(const IR::EmptyStatement*) {
    dump(1);
    builder.endOfStatement();
    return false;
}

bool FPGAToP4::preorder(const IR::IfStatement* s) {
    dump(2);
    builder.append("if (");
    visit(s->condition);
    builder.append(") ");
...
bool FPGAToP4::preorder(const IR::MethodCallStatement* s) {
    dump(3);
    visit(s->methodCall);
    builder.endOfStatement();
    return false;
...
bool FPGAToP4::preorder(const IR::SwitchCase* s) {
    visit(s->label);
    builder.append(": ");
    visit(s->statement);
    return false;
...
bool FPGAToP4::preorder(const IR::SwitchStatement* s) {
    dump(4);
...
bool FPGAToP4::preorder(const IR::Annotation * a) {
    builder.append("@");
...
bool FPGAToP4::preorder(const IR::Parameter* p) {
    dump(2);
    visit(p->annotations);
    switch (p->direction) {
...
bool FPGAToP4::preorder(const IR::P4Control * c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool FPGAToP4::preorder(const IR::ParameterList* p) {
    builder.append("(");
    bool first = true;
...
bool FPGAToP4::preorder(const IR::P4Action * c) {
    dump(2);
    visit(c->annotations);
...
bool FPGAToP4::preorder(const IR::ParserState* s) {
    dump(1);
...
bool FPGAToP4::preorder(const IR::P4Parser * c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool FPGAToP4::preorder(const IR::ExpressionValue* v) {
    dump(2);
    visit(v->expression);
    builder.endOfStatement();
    return false;
...
bool FPGAToP4::preorder(const IR::ActionListElement* ale) {
    dump(3);
...
bool FPGAToP4::preorder(const IR::ActionList* v) {
    dump(2);
    builder.blockStart();
    setVecSep(";\n", ";\n");
...
bool FPGAToP4::preorder(const IR::Key* v) {
    dump(2);
    builder.blockStart();
...
bool FPGAToP4::preorder(const IR::Property* p) {
    dump(1);
    visit(p->annotations);
    if (p->isConstant)
        builder.append("const ");
...
bool FPGAToP4::preorder(const IR::TableProperties* t) {
...
bool FPGAToP4::preorder(const IR::EntriesList *l) {
    dump(1);
    builder.append("{");
    builder.newline();
    builder.increaseIndent();
...
bool FPGAToP4::preorder(const IR::Entry *e) {
    dump(2);
    builder.emitIndent();
...
bool FPGAToP4::preorder(const IR::P4Table* c) {
    dump(2);
    visit(c->annotations);
...
bool FPGAToP4::preorder(const IR::Path* p) {
